...
## Architecture — do not conflate these four paths
...
- **`GET /api/transactions` and `POST /api/transactions/import` are a deliberate, scoped exception to the "no REST CRUD" anti-pattern below** — transactions-display/import-only, not used by the agent. Don't treat it as license to add a symmetric `GET`/`POST /api/conversations`; that anti-pattern still stands.
- **CSV import is all-or-nothing, never partial.** `db._parse_import_csv` validates every row before `import_transactions` writes anything — one bad `transaction type`/`date`/`amount` rejects the whole file with a (1-indexed, original-file) row-numbered error, nothing touches Mongo. Don't change this to skip bad rows or coerce them to a default; silently miscategorized spending is worse than a rejected upload.
- **Accounts are no longer a fixed set of 3 — an import creates exactly one, named/typed by the user in the import dialog, and fully replaces whatever account/transactions existed before** (`accounts.delete_many({})` + `insert_one`, not an upsert into a fixed id) — see specs.md Phase 5 (rewritten). `db.ACCOUNT_TYPES` is just the icon/type choices offered in that dialog, not a set of fixed account identities. Don't reintroduce a fixed `ACCOUNT_IDS`-style list without revisiting this decision.
...
- **Keep test coverage lean, not exhaustive.** One happy-path test plus (only where correctness-critical, e.g. money math on a write path) one failure-path test per new function/endpoint — not a full case matrix, and don't re-run the full suite after every small edit (targeted file first, one full pytest/vitest/tsc pass before calling a change done). `backend/tests/test_chat.py` covers `/api/chat`'s validation/malformed-JSON/502 paths (mocked/hermetic except one `@pytest.mark.live_llm` test — see `backend/README.md`). `backend/tests/test_db_transactions.py`, `test_agent_tools.py`, `test_transactions_endpoint.py` (Phase 4), and `test_import_transactions.py` (Phase 5) cover the transaction/account `db.py` functions, their `agent.py` tool dispatch, `GET /api/transactions`, and CSV import — all hermetic via monkeypatching (`asyncio.run()` inside plain `def` tests, no `pytest-asyncio` dependency added). `ui/src/**/*.test.{ts,tsx}` covers `InputField`, `api.ts`, `storage.ts`, `transactions.ts`, `TransactionsPanel` (account filter + CSV import), `spending.ts`, and `CategorySpendingChart`. Still not covered: the pre-existing `list_conversations`/`search_history`/`delete_conversation` tool dispatch and `db.py` functions, and `MessageList`/`ChatWindow`/`App.tsx` — add lean tests for these if you touch them, don't backfill speculatively.
...
backend/   main.py (routes+validation), agent.py (Ollama calls, tool loop), db.py (Motor client, auto-save, account/transaction queries)
           ...
           Dockerfile, .dockerignore, .env.production(.example) (Phase 6 self-hosted deployment, see DEPLOYMENT.md)
...
ui/        Dockerfile, nginx.conf, .dockerignore (Phase 6, see DEPLOYMENT.md)
docker-compose.yml, DEPLOYMENT.md   Self-hosted production deployment (backend+frontend containers; MongoDB Atlas
                                     and Ollama stay external, unchanged from local dev) — see backend/specs.md Phase 6
```

**Demo transaction data lives in `backend/data/*.csv` — MongoDB is still the only runtime store the agent/endpoint read from.** (These CSVs are meant to be checked into git; as of the Phase 5 import feature they are still untracked in this working tree — `git add` them before relying on `git status` being clean.) To reset to the demo data, edit the CSVs and re-run the seed script; don't hand-edit MongoDB directly or regenerate the CSVs from a random draw (there's no RNG here — see `backend/specs.md` Phase 4 for why `transactions.csv` stores `days_ago` instead of absolute dates). To load real data instead, use the "Import CSV" button in `TransactionsPanel` (see Phase 5) — it replaces the same MongoDB collections the seed script does, but expects a raw bank-statement export (a `Transaction Type`/`Date Posted`/`Transaction Amount`/`Description` header, `YYYYMMDD` dates, DEBIT/CREDIT-typed rows, no category column) rather than the seed CSV's own shape, and creates a single account whose name/type/opening balance you enter in the import dialog rather than reading them from the file.
